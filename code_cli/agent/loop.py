...
from code_cli.agent.context import ContextManager
from code_cli.config import AgentConfig, ContextConfig
from code_cli.models.messages import Conversation, Message, Role, ToolCall, ToolResult
...
class AgentLoop:
    def __init__(
        ...
        if context_config is not None:
            self.context_manager = ContextManager(
                ...
                compress_threshold=context_config.compress_threshold,
            )
        else:
    ...
    async def run(self, user_message: str) -> AsyncIterator[StreamChunk | ToolResult]:
        ...
        # Add user message (set tokens so context compression can run)
        user_msg = Message(
            role=Role.USER,
            content=user_message,
            tokens=self.provider.count_tokens(user_message),
        )
        self.conversation.add(user_msg)
        ...
        while iteration < self.config.max_iterations:
            ...
            # Add assistant message (set tokens so context compression can run)
            assistant_tokens = self.provider.count_tokens(assistant_content)
            ...
            # Add tool results to conversation (set tokens so context compression can run)
            tool_tokens = sum(self.provider.count_tokens(r.content) for r in results)
            self.conversation.add(
                Message(
                    role=Role.TOOL,
                    content="",
